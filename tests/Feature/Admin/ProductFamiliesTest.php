...
class ProductFamiliesTest extends TestCase
{
    ...
    public function test_company_user_sees_system_families_and_own_families_only(): void
    {
        ...
        $response->assertOk();
        $response->assertSee('Material eletrico');
        $response->assertSee('Familia A1');
    ...
    public function test_company_admin_can_create_subfamily_with_visible_parent(): void
    {
        ...
        $admin = $this->createCompanyUser($company, User::ROLE_COMPANY_ADMIN);
        $systemParent = ProductFamily::query()->where('is_system', true)->where('name', 'Material eletrico')->firstOrFail();
    ...
    public function test_company_admin_cannot_create_duplicate_name_under_same_parent_in_visible_context(): void
    {
    ...
    public function test_company_admin_cannot_update_system_or_other_company_family(): void
    {
        $companyA = $this->createCompany('Empresa A');
        $companyB = $this->createCompany('Empresa B');
        $adminA = $this->createCompanyUser($companyA, User::ROLE_COMPANY_ADMIN);
        $systemFamily = ProductFamily::query()->where('is_system', true)->firstOrFail();
        ...
        $this->actingAs($adminA)
            ->patch(route('admin.product-families.update', $systemFamily->id), [
    ...
    public function test_company_admin_cannot_delete_family_with_children_or_protected_family(): void
    {
        ...
        $systemFamily = ProductFamily::query()->where('is_system', true)->firstOrFail();
        $this->actingAs($admin)->delete(route('admin.product-families.destroy', $systemFamily->id))->assertForbidden();
    }
